...
class AWSCosts::EC2Spot
  ...
  def self.fetch type, region
    result = {}
    ['/spot.js'].each do |uri|
      AWSCosts::Cache.get_jsonp(uri % type, 'http://spot-price.s3.amazonaws.com') do |data|
    ...
    raise "No result for region #{region} while fetching EC2 Spot Pricing"            if result[region].nil?
    raise "No result for #{type} in region #{region} while fetching EC2 Spot Pricing" if result[region][type].nil?
